...
一个基于 **Python Flask** 构建的简易用户信息管理平台，提供用户登录认证、信息展示、搜索、头像上传、个人中心、充值等功能。该项目专注于**安全性最佳实践**，适合作为 Flask 入门学习和安全编码参考。
...
user-management/
│
├── app.py                      # Flask 主应用（路由、认证、注册、搜索、上传、个人中心、充值）
├── data/                       # SQLite 数据库文件目录（自动创建）
├── templates/                  # HTML 模板文件夹
│   ├── base.html               # 基础模板（导航栏、布局）
│   ├── index.html              # 首页（用户信息展示 + 搜索 + 快捷入口）
│   ├── login.html              # 登录页面
│   ├── register.html           # 注册页面
│   ├── upload.html             # 头像上传页面
│   └── profile.html            # 个人中心页面（资料查看 + 充值）
├── static/
│   ├── css/
│   │   └── style.css           # 全局样式文件
...
- 提供充值表单，输入金额即可为当前账户充值
- 充值金额必须为正数，金额实时更新到数据库
- 用户仅可查看和充值自己的账户，无法查看其他用户资料
- 未登录用户访问 `/profile` 会自动跳转到登录页
...
- 个人中心和充值功能基于 Session 识别用户身份，不依赖客户端传入的 user_id 参数
- 用户只能查看和操作自己的账户
...
| `/profile` | GET | 显示当前登录用户的个人中心和充值页面（需登录） |
