...
一个基于 **Python Flask** 构建的简易用户信息管理平台，提供用户登录认证、信息展示、搜索、头像上传等功能。该项目专注于**安全性最佳实践**，适合作为 Flask 入门学习和安全编码参考。
...
user-management/
│
├── app.py                      # Flask 主应用（路由、认证、注册、搜索、上传）
├── data/                       # SQLite 数据库文件目录（自动创建）
├── templates/                  # HTML 模板文件夹
│   ├── base.html               # 基础模板（导航栏、布局）
│   ├── index.html              # 首页（用户信息展示 + 搜索 + 上传入口）
│   ├── login.html              # 登录页面
│   ├── register.html           # 注册页面
│   └── upload.html             # 头像上传页面
├── static/
